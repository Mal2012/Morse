...
void uart_init(void)
{
	...
	UCA0MCTLW = 0x10|UCOS16; 			//UCBRFx=1,UCBRSx=0, UCOS16=1
	UCA0CTL1 &= ~UCSWRST; 				//USCI state machine
	P2IE |= UCRXIE; 					//Enable USCI_A0 RX interrupt
...
void uart_putc(unsigned char c)
{
	...
	P2IE|= UCTXIE; 					//Enable USCI_A0 TX interrupt
...
#pragma vector = USCI_A1_VECTOR		//UART TX USCI Interrupt
__interrupt void USCI0TX_ISR(void)
{
	UCA0TXBUF = tx_char;				//Copy char to the TX Buffer
	tx_flag = 0;						//ACK the tx_flag
	P2IE &= ~UCTXIE; 					//Turn off the interrupt to save CPU
}

#pragma vector = USCI_A1_VECTOR		//UART RX USCI Interrupt. This triggers when the USCI receives a char.
__interrupt void USCI0RX_ISR(void)
{
	rx_char = UCA0RXBUF;				//Copy from RX buffer, in doing so we ACK the interrupt as well
	rx_flag = 1;						//Set the rx_flag to 1

	PJOUT ^= BIT1;						//Notify that we received a char by toggling LED
}
